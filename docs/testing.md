...
Upload failure/cancellation uses a test-only remote `tee` wrapper: it writes the first chunk, records a readiness marker, then fails or stalls while an older binary already occupies the installation destination. Both paths must preserve that binary's version/hash. Ctrl+C must interrupt installation. Depending on when Bubble Tea handles SIGINT while resuming the terminal, herdrctx either shows the interrupted-attach dialog or exits with its explicit program-interrupted error (status 1). The test checks these specific outcomes, records the result, and verifies remote preservation in both cases; unrelated errors or exits fail. `interrupted-install.txt` records temporary files and remote processes before teardown; neither killing SSH nor returning to the app proves remote cleanup. Container removal disposes of those remnants and is checked by the fixture. Private-key scanning remains active on all paths.
