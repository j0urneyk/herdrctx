...
1. Set `version` in [herdr-plugin.toml](../herdr-plugin.toml) to the intended binary version. The release tag must be exactly `v<manifest-version>`; the workflow enforces this with the Go `TestReleaseVersion` integration test. Installer-only changes may keep the binary version; a new binary release may not use a mismatched tag.
2. Check user-visible behavior against both READMEs and [UI behavior](ui.md). Update the [changelog](../CHANGELOG.md) and preserve the same release-note context and contributor credit in the relevant PR or commit text. Keep Go aligned between [go.mod](../go.mod) and [.tool-versions](../.tool-versions), and keep packaging within the [target matrix](#target-matrix).
...
[.goreleaser.yaml](../.goreleaser.yaml) generates the changelog in ascending order and excludes entries beginning with `docs:`, `test:`, or `chore:`. The tracked [CHANGELOG.md](../CHANGELOG.md) is the canonical record of user-visible changes. Add pending changes under `Unreleased`; when preparing an authorized release, move only that release's entries under its exact version and release date, keeping `Unreleased` at the top for remaining or future work. GoReleaser still generates its release body from commits and does not read this file automatically. Review the release commits against the changelog before tagging, then reconcile the published release body at closeout. The workflow publishes automatically after validation; it has no separate release-note approval step.
...
3. Compare the published release notes with the corresponding version section in [CHANGELOG.md](../CHANGELOG.md) and the generated commit changelog. Verify the version/date and contributor credit, correct missing or stale notes, and retain the top `Unreleased` section before declaring the release complete.
4. Run the isolated [public GitHub installation check](testing.md#public-github-installation) with the published tag. It checks the selected manifest and installed binary together. Follow its prerequisites. For an extracted native release binary, the [lifecycle](testing.md#real-herdr-lifecycle) and [local registration](testing.md#local-registration-and-nested-guards) checks accept `-integration.binary=/absolute/path/to/herdrctx` with the direct Go test command.
