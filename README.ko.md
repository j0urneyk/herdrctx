...
검증한 0.8.2/0.9.0 조합에서는 native 접속이 일치하는 바이너리를 `~/.local/bin/herdr`에 설치해도 관리 명령은 PATH의 기존 `herdr`를 사용할 수 있습니다. 이 상태에서 새 세션 생성·재접속·관리를 확인했습니다. 이미 실행 중인 세션은 다릅니다. 0.9.0은 0.8.2 서버 교체와 pane 프로세스 중지를 확인하며, 0.8.2는 실행 중인 0.9.0 서버에 접속하지 못하고 기존 서버를 유지합니다. Herdr 전경에서 설정을 진행하고 재시작 안내를 읽은 뒤 수락하세요. herdrctx는 백그라운드에서 설치·재시작을 승인하지 않습니다.
